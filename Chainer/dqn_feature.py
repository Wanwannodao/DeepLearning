...
class QFunction(chainer.Chain):
    ...
    def __init__(self, n_actions):
        ...
        links = [('fe', FeatureExtractor(n_actions))]
        links += [('fc1', L.Linear(3136, fc_unit, initialW=initializer))]
        links += [('fc2', L.Linear(fc_unit, n_actions, initialW=initializer))]
    ...
    # separated flow
    # this shouled not be used while calculatign grads
    def feature(self, x):
...
def show_feature(f):
    feature = chainer.cuda.to_cpu(f[0])
    for i in xrange(feature.shape[0]):
        ax2 = fig2.add_subplot(8, 8, i+1, xticks=[], yticks=[])
        ax2.imshow(feature[i], cmap='gray', interpolation='nearest')
    fig2.canvas.draw()
...
for episode in range(M):
    ...
    while not done and t < limit:
        ...
        if np.random.rand() < epsilon:
            ...
            if step % feature_dump_freq == 0:
                a = get_greedy_action(Q, state, True)
            else:
